...
class ValentiMeta(MetaFrame):
    """
    Extended MetaFrame for using Valenti Dataset (based on vctk)
...
def get_datasets(meta_dir: str, batch_size: int, num_workers: int,
    ...
    train_file, valid_file = ValentiMeta.frame_file_names[1:]

    # load meta file
    train_meta = ValentiMeta(os.path.join(meta_dir, train_file))
    valid_meta = ValentiMeta(os.path.join(meta_dir, valid_file))
